...
def get_phantomjs():
    ...
    phantomjs = requests.get(url)
    with open('../bin/phantomjs.zip', 'wb') as f:
        f.write(phantomjs.content)

    phantomjs_zip = zipfile.ZipFile('../bin/phantomjs.zip', 'r')
    if sys.platform == 'win32':
        phantomjs_zip.extract('bin/phantomjs.exe', '../bin/')
    else:
        phantomjs_zip.extract('phantomjs-2.1.1-macosx/bin/phantomjs', '../bin/')
    phantomjs_zip.close()
    os.rename('../bin/phantomjs-2.1.1-macosx/bin/phantomjs', '../bin/phantomjs')
    os.remove('../bin/phantomjs.zip')
    shutil.rmtree('../bin/phantomjs-2.1.1-macosx')
    
    # Make executable for user
    st = os.stat('../bin/phantomjs')
    os.chmod('../bin/phantomjs', st.st_mode | stat.S_IXUSR)
